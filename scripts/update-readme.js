...
async function main() {
  ...
  let repos = [];
  while (true) {
    const batch = await fetchJson(`https://api.github.com/users/${username}/repos?per_page=${per_page}&page=${page}`, token);
  ...
  try {
    ...
    const res = await fetch(searchUrl, {
      headers: token
        ? { Authorization: `token ${token}`, Accept: 'application/vnd.github.cloak-preview' }
